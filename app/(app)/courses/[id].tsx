...
export default function CourseDetailsScreen() {
  ...
  const fetchCourseDetails = async () => {
    ...
    try {
      ...
      // 2. ATTEMPT ONLINE SYNC (If network claims to be reachable)
      if (netInfo?.isInternetReachable) {
        console.log('✅ Online: Attempting to sync server time...');
        try {
          // fetch from API
          const apiServerTime = await getServerTime(true);
          
          if (apiServerTime) {
            // SUCCESS: We are truly online and synced
            const serverTimeDate = new Date(apiServerTime);
            setServerTime(serverTimeDate);
            setTimeManipulationDetected(false);
            isSessionValid = true;
            console.log('✅ Server time synced. Session valid.');

            // Check if we need to clear a previous flag
            const wasFlagged = await checkManipulationHistory(fetchedUserEmail);
            if (wasFlagged) {
              await clearManipulationFlag(fetchedUserEmail);
            }
          } else {
            throw new Error('Server returned null time');
          }
        } catch (networkError) {
          console.warn('⚠️ Network claimed online but sync failed (flaky connection). Falling back to offline check.');
          // DO NOT BLOCK ACCESS HERE. Fall through to offline check.
        }
      }

      // 3. FALLBACK / OFFLINE CHECK
      // If we aren't online OR if the online sync failed above, we check offline validity.
      if (!isSessionValid) {
        console.log('⚠️ Validating offline access permissions...');
        const canAccess = await canAccessOfflineContent(fetchedUserEmail);
        
        if (canAccess) {
          // SUCCESS: Offline access is allowed
          setTimeManipulationDetected(false);
          isSessionValid = true;
          ...
          // FAILURE: Neither online sync nor offline check passed.
          // This is the ONLY time we block the user.
          console.error('❌ Access Denied: Online sync failed AND offline access invalid/expired.');
          ...
          Alert.alert(
            '🚨 Access Blocked',
            'Your offline access has expired or time manipulation was detected. Please connect to a stable internet connection to restore access.',
            [{ text: 'Understood' }]
          );
          setLoading(false);
          return;
      ...
      // 4. FETCH CONTENT (Based on mode)
      if (isSessionValid) {
        if (!usedOfflineMode && netInfo?.isInternetReachable) {
          ...
          try {
            ...
            if (response.status === 200) {
              ...
              // Background save
              saveCourseDetailsToDb(courseData, fetchedUserEmail).catch(e => console.error(e));
            } else {
              throw new Error('API Error');
            }
          } catch (apiError) {
            console.warn('⚠️ API fetch failed, falling back to local DB.');
          ...
          console.log('📂 Loading course details from Local DB...');
          const offlineData = await getCourseDetailsFromDb(Number(courseId), fetchedUserEmail);
          setCourseDetail(offlineData);
        }
      }

      // 5. Load unlocked assessments (always)
      if (fetchedUserEmail) {
          ...
          {(topic.materials.length > 0 || topic.assessments.length > 0) && (
            <View style={styles.nestedItemsContainer}>
              {topic.materials.map(material => {
                ...
                const sectionAccessible = material.is_accessible !== false; 
                
                // Disable if: Not time-available OR Section restricted OR Time manipulation
                const disabled = !available || !sectionAccessible || timeManipulationDetected;
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      !sectionAccessible && styles.restrictedCard,
                    ...
                    onPress={() => {
                      ...
                      if (!sectionAccessible) {
                        Alert.alert('Restricted Access', '⛔This material is not available to your section.');
                        return;
                      }
                      if (!disabled) {
                  ...
                  >
                    <View style={styles.nestedItemContent}>
                      ...
                      <View style={styles.nestedItemRight}>
                        {/* CHANGED: Logic to show Red Ban icon for Section Restriction */}
                        {!sectionAccessible ? (
                          <View style={styles.restrictedBadge}>
                             <Ionicons name="ban" size={16} color="#d93025" />
                          </View>
              ...
              {topic.assessments.map(assessment => {
                ...
                const sectionAccessible = assessment.is_accessible !== false;
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      ...
                      if (!sectionAccessible) {
                        Alert.alert('Restricted Access', '⛔ This assessment is not available to your section.');
                        return;
                      }
                      if (!disabled) {
                        // -- MODIFIED THIS BLOCK --
        ...
        >
          <View style={styles.itemContent}>
            ...
            <View style={styles.itemRight}>
              ...
              {!sectionAccessible ? (
                <View style={styles.restrictedBadge}>
                    <Ionicons name="ban" size={18} color="#d93025" />
                </View>
              ) : (!available && !timeManipulationDetected) ? (
                <View style={styles.lockedBadge}>
