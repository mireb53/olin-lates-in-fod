...
export default function AttemptQuizScreen() {
  ...
  const saveAnswer = async (submittedQuestionId: number) => {
    const user = await getUserData();
    // (Time manipulation check remains here...)
    if (user?.email) {
      const timeCheck = await detectTimeManipulation(user.email);
      if (!timeCheck.isValid) {
        setTimeManipulationDetected(true);
    ...
    try {
      // 1. ALWAYS Save to Local DB (The "Safety Net")
      // This ensures that even if the internet flickers, your answers are on the phone.
      ...
      // 2. ALSO Save to Server (If Online)
      // This ensures the server sees your progress in real-time.
      if (netInfo?.isInternetReachable) {
        let payload: any = {};
        ...
        // We use a silent catch here so if the server lags, the user doesn't get an error popup
        // because we know the data is already safe in the Local DB (Step 1).
        api.patch(`/submitted-questions/${submittedQuestionId}/answer`, payload)
           .catch(e => console.log("Background save failed, but local save is secure."));
      }
  ...
  const syncCompletedOfflineQuiz = async () => {
    ...
    try {
      if (isOffline === 'true' && submittedAssessment.status === 'completed') {
        ...
        const serverSubmissionId = quizToSync.server_submission_id;
        ...
        // --- FIX: Check .status property, not the object itself ---
        if (syncResult.status === 'success') {
          ...
          console.error(`❌ Failed to sync offline ${assessmentType}:`, syncResult.message);
          Alert.alert(
            'Sync Failed',
            `Failed to sync: ${syncResult.message || 'Unknown error'}`,
            [{ text: 'OK' }]
          );
        }
  ...
  const handleFinalizeQuiz = async (isAutoSubmission: boolean = false) => {
    try {
      ...
      // ... (Validation logic regarding unanswered questions stays here) ...
      // Check for unanswered questions if !isAutoSubmission
      if (!isAutoSubmission) {
        ...
        for (let i = 0; i < shuffledQuestions.length; i++) {
          ...
          if (!isAnswered) unansweredQuestions.push(`Q${i + 1}`);
        }

        if (unansweredQuestions.length > 0) {
          Alert.alert('Incomplete', `Please answer: ${unansweredQuestions.join(', ')}`);
          setSubmitting(false);
          return;
        }
      }
      ...
      // Since we force isOffline='true', we primarily use the syncOfflineQuiz path.
      if (netInfo?.isInternetReachable) {
        console.log(`Submitting ONLINE (Unified Path)...`);
        ...
        if (userEmail && assessmentId && submittedAssessment) {
          ...
          // 1. GET THE ID (Crucial step for security)
          const currentAttempt = await getOfflineQuizAttempt(Number(assessmentId), userEmail);
          const serverSubmissionId = currentAttempt?.server_submission_id;
          ...
          const syncResult = await syncOfflineQuiz(
            ...
            startTime,
            endTime,
            serverSubmissionId // <--- Sending the ID
          );
          ...
          if (syncResult.status === 'success') {
            ...
            // If the server rejected it (e.g. 409 conflict), DO NOT delete the local attempt.
            // We save it locally so the user can try syncing again later.
            ...
            if (!isAutoSubmission) Alert.alert('Sync Error', 'Saved locally. Please try syncing again from the dashboard.');
          }
        }
      } else {
        // Offline Submission
      ...
      console.error(error);
      if (!isAutoSubmission) Alert.alert('Error', 'Submission failed.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleScroll = (event: any) => {
    ...
    setShowStickyTimer(scrollY > 100);
  };

  return (
    <>
      {/* Sticky Timer Bar */}
      {showStickyTimer && assessment.duration_minutes && submittedAssessment.status === 'in_progress' && !timeManipulationDetected && (
        <View style={styles.stickyTimerBar}>
          <Text style={styles.stickyTimerLabel}>⏱️ Time Remaining:</Text>
          <Text style={[
            styles.stickyTimerText,
            timeLeft !== null && timeLeft < 300 && styles.stickyTimerWarning
          ]}>
            {formatTime(timeLeft)}
          </Text>
        </View>
            ...
            {currentPage < Math.ceil(shuffledQuestions.length / QUESTIONS_PER_PAGE) - 1 ? (
              <TouchableOpacity onPress={() => setCurrentPage(p => p + 1)} style={styles.navButton}>
               ...
               submittedAssessment.status === 'in_progress' && !timeManipulationDetected && !autoSubmitting && isReview !== 'true' && (
                ...
                >
                  {submitting ? (
                    <ActivityIndicator color="#1967d2" />
                  ) : (
                    <Text style={styles.navButtonText}>Submit {assessmentTypeCapitalized}</Text>
                  )}
                </TouchableOpacity>
              )
            )}
