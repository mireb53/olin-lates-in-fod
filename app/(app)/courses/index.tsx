...
export default function CoursesScreen() {
  ...
  const handleDeleteCourse = async (course: EnrolledCourse) => {
    ...
    Alert.alert(
      'Delete Course Data',
      `Are you sure you want to delete the offline content for "${course.title}"?`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Delete',
          ...
          onPress: async () => {
            try {
              // This calling the UPDATED function in localDb.ts
              ...
              Alert.alert(
                'Success',
                `Offline data for "${course.title}" has been cleaned up. (Pending submissions were preserved)`
              );
            } catch (error) {
              console.error('❌ Error deleting course data:', error);
              Alert.alert('Error', 'Failed to delete course data.');
            }
          }
        }
      ]
