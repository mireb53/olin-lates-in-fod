...
import {
  forceRefreshAllAssessmentStatuses,
...
export default function TodoScreen() {
  ...
  const [recentlySyncedIds, setRecentlySyncedIds] = useState<Set<number>>(new Set());
    ...
    try {
      ...
      for (const assessment of allAssessments) {
        ...
        const todoItem: TodoItem = {
          ...
          recentlySynced: status === 'done' && recentlySyncedIds.has(assessment.id)
        };
  ...
  const runTargetedSync = async (isManualClick = false) => {
    ...
    if (!netInfo?.isInternetReachable) {
      if (isManualClick) Alert.alert("Offline", "Please connect to the internet to sync your work.");
      return;
    }

    if (categoryCounts.to_sync === 0) {
      if (isManualClick) Alert.alert("All Synced", "Your work is already up to date.");
      return;
    }
  
    console.log('🔄 [Targeted Sync] Starting manual sync sequence...');
    setIsSyncing(true);
    ...
    try {
      ...
      // 1. CAPTURE IDs BEFORE SYNC ----------------------------------
      // We need to know WHICH items are about to be synced so we can highlight them later
      const pendingAssignments = await getUnsyncedSubmissions(userData.email);
      const pendingQuizzes = await getCompletedOfflineQuizzes(userData.email);
      
      // Create a temporary set of IDs currently in the "outbox"
      const idsInTransit = new Set<number>();
      pendingAssignments.forEach((item: any) => idsInTransit.add(item.assessment_id));
      pendingQuizzes.forEach((item: any) => idsInTransit.add(item.assessment_id));
      // -------------------------------------------------------------

      const { success, failed } = await manualSync();
  
      if (success > 0) {
        console.log('🔄 [Targeted Sync] Upload success.');
        
        // 2. UPDATE STATE --------------------------------------------
        // If sync succeeded, add the captured IDs to the "recently synced" list
        setRecentlySyncedIds(prev => {
          const newSet = new Set(prev);
          idsInTransit.forEach(id => newSet.add(id));
          return newSet;
        });
        // ------------------------------------------------------------
        ...
        await forceRefreshAllAssessmentStatuses(
          userData.email,
          api,
          (current, total) => console.log(`[Sync Refresh] ${current}/${total}`)
        );
      }

      if (success > 0 && failed === 0) {
        Alert.alert('Sync Complete', `Successfully submitted ${success} item(s). Look for the "Just Synced" badge in the Done tab.`);
      } else if (failed > 0) {
        Alert.alert('Sync Issue', `${failed} item(s) failed to upload. Please try again.`);
      }
              ...
              <View style={styles.courseRow}>
                ...
                {item.recentlySynced && (
                  <View style={styles.syncedBadge}>
                    <Ionicons name="checkmark-done" size={10} color="#fff" style={{marginRight: 2}} />
                    <Text style={styles.syncedText}>JUST SYNCED</Text>
                  </View>
                )}
              </View>
      ...
      <View style={styles.header}>
        <View style={styles.headerContent}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
            ...
            {(hasPendingSync || categoryCounts.to_sync > 0) && netInfo?.isInternetReachable && (
              <TouchableOpacity 
              ...
              >
                {isSyncing ? (
                  ...
                  <>
                    ...
                    <Text style={styles.syncIndicatorText}>Sync Now</Text>
                  </>
                )}
              </TouchableOpacity>
            )}
...
const styles = StyleSheet.create({
  ...
  syncedBadge: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#0d904f', // Google "Green" for success
    paddingHorizontal: 8,
    paddingVertical: 3,
    borderRadius: 4,
    marginLeft: 8,
